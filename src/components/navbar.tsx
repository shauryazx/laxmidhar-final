    ...
    <nav className="sticky top-0 z-50 w-full bg-primary text-primary-foreground border-b border-primary/20 shadow-lg">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between min-h-[100px] py-4">
          {/* Institutional Logo & Stacked Title */}
          <Link href="/" className="flex items-center gap-3 md:gap-4 group shrink-0">
            ...
            <div className="flex flex-col leading-[0.9] font-black uppercase tracking-tighter">
              <span className="text-lg md:text-xl text-white">Laxmidhar</span>
              <span className="text-lg md:text-xl text-white">Polytechnic</span>
              <span className="text-lg md:text-xl text-white">College</span>
              <span className="text-[10px] font-bold text-accent tracking-[0.15em] mt-1">Alwar, Rajasthan</span>
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navItems.map((item) => (
              <Link
                ...
                href={item.href}
                className={cn(
                  "px-3 py-2 rounded-md text-sm font-bold transition-colors hover:text-accent flex items-center gap-2 whitespace-nowrap",
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="text-primary-foreground hover:text-accent hover:bg-transparent flex items-center gap-2 font-bold text-sm">
              ...
              <DropdownMenuContent align="end" className="bg-white text-foreground rounded-xl shadow-2xl border-none p-2">
                {adminItems.map((item) => (
                  <DropdownMenuItem key={item.href} asChild>
                    <Link href={item.href} className="w-full cursor-pointer px-4 py-2 rounded-lg hover:bg-muted font-bold text-sm">
                      {item.name}
                    </Link>
                  </DropdownMenuItem>
                ))}
        ...
        <div className="lg:hidden bg-white text-primary border-t border-border animate-in slide-in-from-top duration-300">
          <div className="px-4 py-4 space-y-2">
            ...
            <div className="pt-4 border-t border-border">
              <span className="px-3 text-xs font-black text-muted-foreground uppercase tracking-widest">Administrator Desks</span>
              {adminItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  onClick={() => setIsOpen(false)}
                  className="block px-3 py-3 rounded-md text-base font-bold hover:bg-accent/5"
                >
                  {item.name}
                </Link>
              ))}
